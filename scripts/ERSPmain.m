...
%%
timeRange.theta=[244 577];
timeRange.alpha=[211 550];
timeRange.beta=[314 592];

scalpObject=ScalpObject(scalpObj.DATA, scalpObj.info, baselineTime, timeRange); % Initialize scalp processing
scalpObject = scalpObject.standardProcessing('outlier','3MAD','calBaseline',0); % standard processing pipeline, 5SD outlier, baseline correction
...
% Scalp Plotting
close all
vars2plot={'emotion'}; freq2plot={'alpha'}; times2plot={'choice'}; errorType='sem';
scalpObject.plotScalpMap('vars2plot',vars2plot,'freq2plot',{'alpha'},'times2plot',{'alpha'});
caxis([-5 5])
